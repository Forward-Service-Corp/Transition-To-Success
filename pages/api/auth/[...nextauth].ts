...
async function findUserByPhone(db: Db, phoneNumber: string) {
...
export const authOptions: NextAuthOptions = {
  ...
  session: {
    ...
    maxAge:
      (parseInt(process.env.SESSION_AUTO_LOGOUT_LENGTH_IN_MINUTES || "") ||
    ...
    Credentials({
      ...
      async authorize(credentials) {
        ...
        if (response === "approved" && userSearch) {
          const userObject = {
            _id: userSearch._id.toString(),
            id: userSearch._id.toString(),
            name: userSearch.name || "",
            email:
              userSearch.email || `phone-${userSearch.phone}@credentials.local`,
  ...
  callbacks: {
    ...
    async signIn({ user, account, credentials }) {
      ...
      if (account.type === "email") {
        ...
        // Use fuzzy phone matching
        const userSearch = await findUserByPhone(db, user.phone);
        if (userSearch) {
          return true;
        } else {
          return "/api/auth/no-account";
        ...
        try {
          ...
          if (account.type === "credentials") {
            ...
            if (user.phone) {
              ...
              user.email &&
              user.email.includes("@credentials.local")
            ) {
              const phoneMatch = user.email.match(
                /phone-([^@]+)@credentials\.local/,
              );
              if (phoneMatch) {
  ...
  events: {
    signIn: async ({ user, isNewUser }) => {
      ...
      if (isNewUser) {
        ...
        const isFSCEmail = userEmail.indexOf("fsc-corp.org") > -1;
