...
export default function CarePlans({
  ...
  pageJson: IndexDataResponse;
}) {
  const [tasks, setTasks] = useState(pageJson.tasks);
  const [userReferrals, setUserReferrals] = useState(pageJson.referrals);
  const [sort] = useState("priority");

  const updateTaskHandler = async (newTasks: TaskResponse[]) => {
    setTasks(newTasks);
  };

  return (
    <Layout title={"CARE Plans"} user={pageJson.user}>
      <Head>
      ...
      <div className={`flex justify-between align-middle items-center`}>
        <div>
          <h2 className={"uppercase text-gray-500 my-4 print:hidden"}>
            Manage Care Plans
          </h2>
        </div>
        <button
      ...
      {userReferrals
        ...
        .sort((a, b) => {
          ...
          if (sort === "domain") {
            ...
            if (a.priority !== null) {
              aSort = parseInt(a.priority || "");
            } else {
              aSort = 0;
            }
            if (b.priority !== null) {
              bSort = parseInt(b.priority || "");
            } else {
              bSort = 0;
            }
            return bSort - aSort;
      ...
      <h2 className={"uppercase text-gray-500 mb-4 mt-10 print:hidden"}>
        Archived Care Plans
      </h2>
      {userReferrals
        .filter(
          (item) => item.hasOwnProperty("archived") && item.archived === "true",
        )
        .sort((a, b) => {
          let aSort: number;
          let bSort: number;
...
export async function getServerSideProps(context: NextPageContext) {
  ...
  try {
    ...
    const pageJson: IndexDataResponse = await getPageData.json();
