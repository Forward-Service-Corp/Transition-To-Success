...
export default function LifeAreaSurveys({ user, surveys, incomingDream }) {
  ...
  return (
    <Layout title={"Life Area Surveys"} user={user}>
      <Head>
        <title>TTS / Life Area Surveys</title>
      </Head>
      <div className={"w-full border-b-[1px] pb-2 mb-5"}>
        <h2 className={"uppercase text-orange-600 "}>
          Active Life Area Survey
        </h2>
      ...
      <div className={"w-full border-b-[1px] pb-2 mb-5 mt-10"}>
        <h2 className={"uppercase text-orange-600 mb-5"}>
          Life Area Survey History
        </h2>
      </div>
      <LasHistory
        user={user}
...
export async function getServerSideProps(context) {
  ...
  try {
    // page data
    const dataUrl =
      baseUrl + "/api/pages/surveysPageData?userId=" + session.user._id;
