    ...
    const saveSurveyButton = () => {
        return (
            <button disabled={domains.length === 0 || Object.keys(answered).length !== 21}
                    ...
                    }}>{domains.length === 0 && Object.keys(answered).length !== 21 ? "Score all domains and select at least one Priority" : (domains.length === 0 ? "Select at least one priority" : (Object.keys(answered).length !== 21 ? "Please score all domains" : "Save this Survey"))}
            </button>
        )
    }
