...
type LayoutProps = {
  ...
  simpleModalTitle?: string;
  simpleModalMessage?: string;
  simpleModalLabel?: string;
  background?: boolean;
...
export default function Layout({
  ...
  //const { showWarning, timeRemaining, extendSession, handleLogout } = useAutoLogout(user)

  const handleManualLogout = async () => {
    // Properly handle logout with user validation
    if (user) {
  ...
  useEffect(() => {
    ...
    const now = Date.now();
    const lastLoginTime = new Date(user?.lastLogin || now).getTime(); // Convert lastLogin to a Date object
  ...
  return (
    <>
      ...
      >
        <div
          className={`${version ? styles.youthVersion : styles.adultVersion} pb-32 print:hidden`}
        >
          ...
          <SubNav session={user} handleLogout={handleManualLogout} />
          <header className="py-10">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              >
                {version ? "Youth Workbook" : "Adult Workbook"}
              </p>
            </div>
          </header>
        </div>
