...
function NewPhoneAccount({ loginValue, setStep }) {
    ...
    return (
      <div>
        <label htmlFor={name} className={`block mb-2 font-bold`}>
          {label}
          <span className={`text-red-600 ${required ? "visible" : "hidden"}`}>
            *
          </span>
        </label>
        <input
          ...
          name={name}
          autoComplete="true"
          className={`rounded mb-4 text-xs w-full ${
            formValidationData[name] === null
              ? ""
              : formValidationData[name]
              ? "border-green-600"
              : "border-gray-300"
          }`}
  ...
  const selectJSX = (
    ...
    return (
      <div className={""}>
        <label htmlFor={name} className={"text-xs w-full block mb-2 font-bold"}>
          {label}
          <span className={`text-red-600 ${required ? "visible" : "hidden"}`}>
            *
          </span>
        ...
        >
          ...
          {list.map((currentCounty) => {
            return (
              <option key={currentCounty} value={currentCounty}>
                {currentCounty}
              </option>
            );
          })}
  ...
  const multipleSelectJSX = (
    ...
    return (
      <div className={""}>
        <label htmlFor={name} className={"text-xs w-full block font-bold"}>
          {label}
          <span className={`text-red-600 ${required ? "visible" : "hidden"}`}>
            *
          </span>
        </label>
        <span className={`text-indigo-600 block mb-2`}>
          Control + Click to select multiple options
        </span>
        <select
          className={`rounded text-xs w-full border-2 ${getFieldClass(
            required,
            value
          )}`}
          id={name}
  ...
  const updateFormValidationData = (name, value) => {
    setFormValidationData((prevData) => {
      ...
      let allValid;
      allValid = Object.keys(updatedData).every(
    ...
    setUserData((prevData) => {
      ...
      for (let key in newData) {
        if (Array.isArray(newData[key])) {
          ...
          if (
            newData[key].length !== prevData[key].length ||
  ...
  const sendOTP = async () => {
    await fetch(`/api/send-OTP?phone=${loginValue}`).then((res) => res.json());
  };
  ...
  return (
    <div className={`text-xs`}>
      <div className={``}>
        <h2 className={"pb-4 font-extralight text-2xl my-2 w-full text-center"}>
          Enter Required Information
        </h2>
        <div className={`text-xl mt-3 mb-2`}>Personal Details</div>
        <form className={``} onSubmit={checkUsedEmail}>
          ...
          <div className={``}>
            <div className={`text-xl mt-3 mb-2`}>Programs</div>
            <div className={`grid grid-cols-2 gap-x-8`}>
              {multipleSelectJSX(
                "Service Counties",
                "county",
          ...
          <div
            className={`${
              usedEmail ? "visible" : "hidden"
            } bg-red-200 p-2 text-center mt-4 rounded`}
          >
            There is already an account associated with{" "}
            <span className={"font-bold"}>{userData.email}</span>. Please use a
            different email address, or sign in with that email address.
          </div>
          <div className={`text-center`}>
            <div className={"w-full p-8"}>
              <button
                className={
                  "rounded bg-indigo-600 text-white font-extralight p-2 disabled:bg-gray-300 w-1/2 text-lg"
                }
                disabled={!formValid}
                type={`submit`}
              >
              ...
              <Link
                href="/login"
                className={`text-red-600 underline mt-5 text-sm block m-auto`}
              >
                Go Back
              </Link>
            </div>
